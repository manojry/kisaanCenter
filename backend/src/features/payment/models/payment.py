from sqlalchemy import Column, Integer, String, Text, Boolean, DateTime, Date, ForeignKey, Enum, DECIMAL, JSON
from sqlalchemy.orm import relationship
from datetime import datetime
from ....models import PaymentType, FarmerPaymentType, RecordStatus
...
class Payment(Base):
    ...
    reference_number = Column(String(100), nullable=True)  # For tracking external payments
...
class FarmerPayment(Base):
    ...
    def is_approved(self):
        """Check if farmer payment is approved"""
        return self.approved_by is not None and self.status == RecordStatus.ACTIVE
    ...
    def get_farmer_name(self):
        """Get farmer name if relationship is loaded"""
        if hasattr(self, 'farmer_user') and self.farmer_user:
            return self.farmer_user.username
        return f"Farmer ID: {self.farmer_user_id}"
