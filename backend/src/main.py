...
# Initialize database engine immediately when module is imported
try:
    if db_manager._engine is None:
        db_manager.initialize_engine()
        logger.info("📊 Database engine initialized on import")
    else:
...
async def lifespan(app: FastAPI):
    ...
    try:
        # Test database connection (engine already initialized)
...
# Create essential endpoints - these should always be available regardless of router imports
...
async def get_owner_dashboard(shop_id: int = 1, db: Session = Depends(get_db)):
    ...
    for txn in transactions:
        if hasattr(txn, 'transaction_items') and txn.transaction_items:
            txn_amount = sum(float(item.quantity * item.price) for item in txn.transaction_items if item.quantity and item.price)
...
# Add the endpoint that the frontend expects
@app.get("/api/v1/owner/analytics/dashboard")
async def get_owner_analytics_dashboard(shop_id: int = 1, db: Session = Depends(get_db)):
    from src.models import Shop, Product, Transaction, User, FarmerStock
    from datetime import date
    ...
    return {
        ...
        "shop_transactions": total_transactions,
        "shop_products": total_products,
        "today_transactions": today_transactions,
        "monthly_growth": "+12%",  # This would be calculated based on historical data
...
async def get_products(shop_id: int = 1, db: Session = Depends(get_db)):
    try:
        ...
        # Fallback to direct query without status filter
...
async def get_transactions(shop_id: int = 1, db: Session = Depends(get_db)):
    ...
    transaction_data = []
    for txn in transactions:
        # Calculate total amount from transaction items safely
        total_amount = 0
        if hasattr(txn, 'transaction_items') and txn.transaction_items:
            total_amount = sum(float(item.quantity * item.price) for item in txn.transaction_items if item.quantity and item.price)
        
        transaction_data.append({
            "id": txn.id,
            "amount": total_amount,
            "status": txn.status.value if hasattr(txn, 'status') and txn.status else 'active',
            "shop_id": txn.shop_id,
...
# ===================================================================
# ESSENTIAL ENDPOINTS - Always available regardless of import issues  
# ===================================================================
