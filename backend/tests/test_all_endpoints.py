...
class TestAPIEndpoints:
    ...
    def teardown_class(cls):
        ...
        # Remove test shop
        try:
            if 'shop_id' in cls.test_data:
                response = requests.delete(f"{BASE_URL}/shops/{cls.test_data['shop_id']}", headers=cls.headers)
                if response.status_code == 200:
                    print(f"✅ Test shop {cls.test_data['shop_id']} deleted successfully")
                else:
                    print(f"⚠️ Could not delete test shop: {response.text}")
        except Exception as e:
            print(f"Warning: Could not delete test shop: {e}")

        # Remove test user
        try:
            if 'new_user_id' in cls.test_data:
                response = requests.delete(f"{BASE_URL}/users/{cls.test_data['new_user_id']}", headers=cls.headers)
                if response.status_code == 200:
                    print(f"✅ Test user {cls.test_data['new_user_id']} deleted successfully")
                else:
        ...
        # Remove test product
        try:
            if 'product_id' in cls.test_data:
                response = requests.delete(f"{BASE_URL}/products/{cls.test_data['product_id']}", headers=cls.headers)
                if response.status_code == 200:
                    print(f"✅ Test product {cls.test_data['product_id']} deleted successfully")
                else:
