...
export function success<T = any>(res: Response, data: T, opts?: SuccessOpts) {
  const { message, aliasKey, meta, statusCode } = opts || ({} as any);
...
export function created(res: Response, data: any, opts: Omit<SuccessOpts,'statusCode'> = {}) {
  return success(res, data, { ...opts, statusCode: 201 });
}

// Generic failure responder (backward compatible). "error" may be any string.
export function failure(res: Response, statusCode: number, error: string | ErrorCode, details?: any, message?: string) {
...
export function failureCode(res: Response, statusCode: number, code: ErrorCode, details?: any, message?: string) {
