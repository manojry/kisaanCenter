...
module.exports = {
  ...
  const showTables = await queryInterface.showAllTables() as any[];
  const normalized = showTables.map((t: any) => (t && typeof t === 'object' && t.tableName) ? t.tableName : String(t));
    ...
    const txnDesc: any = await queryInterface.describeTable('kisaan_transactions');
