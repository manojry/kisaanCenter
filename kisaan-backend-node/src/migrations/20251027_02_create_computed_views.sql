...
-- Idempotent: Drop view if exists (will be recreated with correct dependencies)
DROP VIEW IF EXISTS v_user_balance_validation CASCADE;
DROP VIEW IF EXISTS v_expense_settlement_status CASCADE;
DROP VIEW IF EXISTS v_transaction_settlement_status CASCADE;

-- Only create if payment_allocations table exists (dependency check)
DO $$
BEGIN
  IF EXISTS (
    ...
    CREATE VIEW v_transaction_settlement_status AS
    SELECT 
    ...
    GROUP BY t.id;
  ELSE
    RAISE WARNING 'Skipping v_transaction_settlement_status view: payment_allocations table does not exist yet';
  END IF;
END $$;
