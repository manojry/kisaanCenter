...
module.exports = {
  up: async (queryInterface: QueryInterface) => {
    ...
    const tables = await queryInterface.showAllTables() as any[];
    const names = tables.map((t: any) => (t && typeof t === 'object' && t.tableName) ? t.tableName : String(t));
    ...
    if (allocTable) {
      ...
      try {
        ...
        console.warn('[migration] unique index create failed (may already exist):', e?.message || e);
      }
    } else {
  ...
  down: async (queryInterface: QueryInterface) => {
    ...
    if (allocTable) {
      try {
        ...
        console.warn('[migration:down] drop unique index failed:', e?.message || e);
      }
    }
  }
};
