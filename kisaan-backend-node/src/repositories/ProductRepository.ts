...
export class ProductRepository extends BaseRepository<Product, ProductEntity> {
  ...
  protected toDomainEntity(model: Product): ProductEntity {
    return new ProductEntity({
      ...
      category_id: (model as any).category_id ?? undefined,
      description: (model as any).description ?? null,
      unit: typeof (model as any).unit === 'string' ? (model as any).unit : undefined,
    });
  }
   ...
   */
  protected toModelData(entity: Partial<ProductEntity>): any {
